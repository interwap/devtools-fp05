...
import com.praescient.components.fgtit_fp05.utils.DataUtils;
...
public class AsyncFingerprint extends Handler {
    ...
    /**
     * The response packet and the image data are 40044 bytes
     */
    ...
    /**
     * The response packet and eigenvalue data are 568 bytes
     */
    private static final int UP_CHAR_RESPONSE_SIZE = 568;

    /**
     *  Buffer: Lower computer to return to the data storage
     */
    ...
    /**
     * Enter the fingerprint image
     *
     * @return Return value is confirmation code Confirmation code = 00H Input successful confirmation code = 01H Packed with error confirmation code = 02H That there is no finger on the sensor
     *         Confirmation code = 03H means the entry is unsuccessful Confirmation code = -1 means failure
     */
    ...
    /**
     * Fingerprint image generated in ImageBuffer feature file stored in charBuffer1 or CharBuffer2
     *
     * @param bufferId
     *            （CharBuffer1:1h,CharBuffer2:2h）
     * @return Return value for the confirmation code confirmation code = 00H that the characteristics of the success of the generation of confirmation code = 01H that received a packet error confirmation code = 06H that the fingerprint image is too messy and not an image
     *         Acknowledge Code = 07H Indicates that the fingerprint image is normal, but the feature point is too small to generate a character. Acknowledgment Code = 15H Indicates that there is no valid raw image in the image buffer.
     *         Indicates failure
     */
    ...
    /**
     * Merge feature generation template, the Char Buffer1 and CharBuffer 2 in the merger of the characteristics of the template file, the results stored in CharBuffer1 and CharBuffer2
     * in.
     *
     * @return Return value is confirmation code confirmation code = 00H that merged successfully confirmed code = 01H that received a packet error code = 0aH that merged failed (two fingerprints do not belong to the same finger)
     *      Acknowledgment code = -1 for failure
     */
    ...
    /**
     * Save the template in CharBuffer to the flash database location of the specified page ID number bufferId: only 1h or 2h
     * The pageId argument ranges from 0 to 1010
     *
     * @return Return value is confirmation code Confirmation code = 00H Said storage success confirmation code = 01H That the packet has a mistake Confirmation code = 0bH That the PageID beyond the fingerprint library range
     *         Acknowledgment Code = 18H Indicates Write Error Code = -1 indicates failure
     */
    ...
    /**
     * Will be specified in the flash database pageId number of the template read into the template buffer CharBuffer1 or CharBuffer2
     * bufferId:只能为1h或2h pageId：范围为0~1023
     *
     * @param
     *
     * @return Return value is the confirmation code confirmation code = 00H that read successful confirmation code = 01H that the packet has a wrong code = 0cH read error or template error
     *         Acknowledgment Code = 0BH Indicates that the PageID is out of the fingerprint library. Acknowledgment code = -1 indicates failure
     */
...
//		Log.i("whw", "sendCommand hex=" + DataUtils.toHexString(command));
    ...
    private void printlog2(String tag, int length) {
        byte[] temp = new byte[length];
        System.arraycopy(buffer, 0, temp, 0, length);
        Log.i("xxx", tag + "=" + DataUtils.toHexString(temp));
    }
